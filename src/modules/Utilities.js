import {
  subDays, addDays, format, isWithinInterval,
} from 'date-fns';
...
function isInInterval(dueDate, range) {
  ...
  return isWithinInterval(new Date(`${date[0]}/${date[1]}/${date[2]}`), {
...
function isValidFormat(projectName) {
  return (
    projectName.match(/([12]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]))/)
    === null
  );
}
...
function extractComponents(title) {
  ...
  return idx !== -1
    ? [title.substring(0, idx - 1), title.substring(idx + 1, title.length - 1)]
    : [title];
}
